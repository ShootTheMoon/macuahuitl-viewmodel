...
if false and (_G.UltCharge or 0) < 1 then
		return
	end
	...
	if MELEE.isMaxico() then
		if not getClip("Transform") or not getClip("UltAttack") then
			print("[ViewmodelController] Maxico V10 궁극기 클립 없음")
			return
		end
	...
	if MELEE.ruinT then
		...
		if cam then
			...
			if MELEE.ruinT >= R.DELAY then
				...
				fireServerAttack(1)
			elseif MELEE.ruinT >= (R.DELAY - R.AIM) then
					...
					if pl ~= LocalPlayer and not w.hit[pl] then
						...
						if r then
							...
							if along > 0 and along <= front
								and (v - w.dir * along).Magnitude <= R.HIT_R then
								w.hit[pl] = true
								pcall(function()
...
function MELEE.jumongStep(dt)
	...
	if flyingRaijinButton and MELEE.chargeTime() then
		...
		pcall(function()
			flyingRaijinButton.ImageColor3 = col
			flyingRaijinButton.ImageTransparency = tr
		end)
	end
	...
	MELEE.glowNow = MELEE.glowStage()
		...
		for _, item in pairs(viewmodelPartsByName) do
			...
			if part.Parent then
				...
				elseif part.Name == "Arrow_Gukgung" then
...
function MELEE.attackDown()
		...
		task.spawn(function()
			while _G.InLobby ~= false do
				task.wait(0.1)
			end
			task.wait(1)
			_G.UltCharge = 1
			onUltPressed()
			print("[VMTEST] maxico ult started")
		end)
